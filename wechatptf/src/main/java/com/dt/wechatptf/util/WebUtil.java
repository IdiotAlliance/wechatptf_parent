...
public class WebUtil {
	...
	public static Map<String, String> getHeaders(
		...
		final Enumeration<String> hkeys = request.getHeaderNames();
		while (hkeys.hasMoreElements()) {
			final String hkey = hkeys.nextElement();
			final String value = request.getHeader(hkey);
			headers.put(hkey, value);
		}
		return headers;
	}

	public static String getHeader(final String hname,
			final HttpServletRequest request) {
		if (hname == null || request == null)
			throw new NullPointerException();

		final Enumeration<String> hkeys = request.getHeaderNames();
		while (hkeys.hasMoreElements()) {
			if (hname.equals(hkeys.nextElement()))
				return request.getHeader(hname);
		}
		return null;
	}

	public static boolean isFromMobileBrowser(final HttpServletRequest request) {
